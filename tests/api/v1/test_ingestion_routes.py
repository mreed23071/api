...
async def test_a_run_returns_the_report_shape(client_with_source) -> None:
    ...
    body = response.json()
    expected = {
        "run_id", "started_at", "finished_at", "duration_ms", "dry_run",
        "fetched", "already_ingested", "evaluated", "retained", "discarded",
        "filter_errors", "embedded", "persisted", "users_provisioned",
        "relations_provisioned", "filter_provider", "filter_prompt_version",
        "embedding_model", "decisions",
    }
...
async def test_dry_run_persists_nothing(client_with_source) -> None:
    response = await client_with_source.post(
        RUNS, json={"dry_run": True}, headers=INGEST_HEADERS
    )
    body = response.json()
...
async def test_limit_is_validated_at_the_edge(client_with_source) -> None:
    response = await client_with_source.post(
        RUNS, json={"limit": 0}, headers=INGEST_HEADERS
    )
